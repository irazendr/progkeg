...
<div id="layoutSidenav_content">
                            ...
                            <?php foreach ($data_mitra as $mitra) : ?>
                                <tr>
                                    ...
                                    <td width="15%" class="text-center">
                                        <button type="button" class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#ubahModal<?= $mitra->id_mitra; ?>"><i class="fas fa-edit"></i>
                                        </button>
                                        <button type="button" class="btn btn-danger btn-sm" onclick="hapus(<?= $mitra->id_mitra; ?>)"><i class="fas fa-trash-alt"></i>
                                        </button>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
    ...
    <?php foreach ($data_mitra as $l) : ?>
        <!-- Modal Ubah -->
        <div class="modal fade" id="ubahModal<?= $l->id_mitra; ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header bg-modal text-white">
                    ...
                    <div class="modal-body">
                        <form action="<?= base_url('mitra/ubah/' . $l->id_mitra); ?>" method="post">
                            <?= csrf_field(); ?>
                            <input type="hidden" name="_method" value="PUT">
                            <div class="mb-3">
                                <label for="id_mitra">ID Mitra</label>
                                <input type="text" name="id_mitra" id="id_mitra" class="form-control" value="<?= $l->id_mitra; ?>" disabled>
                            </div>
                            <div class="mb-3">
                                <label for="nama_lengkap">Nama Lengkap</label>
                                <input type="text" name="nama_lengkap" id="nama_lengkap" class="form-control" value="<?= $l->nama_lengkap; ?>" required>
                            </div>
                            <div class="mb-3">
                                <div class="form-floating mb-3 mb-md-0">
                                    <select class="form-select" aria-label="Default select example" id="role" name="role">
                                        <?php foreach ($list_role as $m) : ?>
                                            <option value="<?= $m->id; ?>" <?php if ($m->id == $l->role) : ?>selected<?php endif ?>>
                                                <?= $m->role_petugas; ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                    <label for="role">Role</label>
